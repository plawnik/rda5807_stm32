...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
  ...
  /* Pull D+ low briefly before handing PA11/PA12 to USB. This forces a clean
   * re-enumeration on boards with a fixed external 1.5 kohm D+ pull-up. */
  gpio.Pin = USB_DP_Pin;
  gpio.Mode = GPIO_MODE_OUTPUT_PP;
  gpio.Pull = GPIO_NOPULL;
  gpio.Speed = GPIO_SPEED_FREQ_HIGH;
  HAL_GPIO_Init(USB_DP_GPIO_Port, &gpio);
...
void *USBD_static_malloc(uint32_t size)
{
  ...
  if (size > sizeof(mem)) return NULL;
  return mem;
}
